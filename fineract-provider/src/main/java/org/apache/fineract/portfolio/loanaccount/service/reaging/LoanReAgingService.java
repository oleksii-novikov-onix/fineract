...
import org.apache.fineract.portfolio.loanaccount.api.ReAgePreviewQueryParams;
...
public class LoanReAgingService {
    ...
    public LoanScheduleData previewReAge(final Long loanId, final String loanExternalId, final ReAgePreviewQueryParams queryParams) {
        ...
        return previewReAge(loan, queryParams);
    }

    private LoanScheduleData previewReAge(final Loan loan, final ReAgePreviewQueryParams queryParams) {
        reAgingValidator.validateReAge(loan, queryParams);

        final LoanTransaction reAgeTransaction = createReAgeTransactionFromQueryParams(loan, queryParams);
    ...
    private LoanTransaction createReAgeTransactionFromQueryParams(Loan loan, ReAgePreviewQueryParams queryParams) {
        LocalDate transactionDate = DateUtils.getBusinessLocalDate();
        final Locale locale = queryParams.getLocale() != null ? Locale.forLanguageTag(queryParams.getLocale()) : Locale.getDefault();
        final LocalDate startDate = JsonParserHelper.convertFrom(queryParams.getStartDate(), LoanReAgingApiConstants.startDate,
                queryParams.getDateFormat(), locale);
        if (transactionDate.isAfter(startDate)) {
            transactionDate = startDate;
        }
        ...
        final LoanReAgeParameter reAgeParameter = createReAgeParameterFromQueryParams(reAgeTransaction, queryParams);
        reAgeTransaction.setLoanReAgeParameter(reAgeParameter);

        return reAgeTransaction;
    }

    private LoanReAgeParameter createReAgeParameterFromQueryParams(final LoanTransaction reAgeTransaction,
            final ReAgePreviewQueryParams queryParams) {
        final PeriodFrequencyType periodFrequencyType = PeriodFrequencyType.valueOf(queryParams.getFrequencyType());
        final Locale locale = queryParams.getLocale() != null ? Locale.forLanguageTag(queryParams.getLocale()) : Locale.getDefault();
        final LocalDate startDate = JsonParserHelper.convertFrom(queryParams.getStartDate(), LoanReAgingApiConstants.startDate,
                queryParams.getDateFormat(), locale);
        final Integer numberOfInstallments = queryParams.getNumberOfInstallments();
        final Integer periodFrequencyNumber = queryParams.getFrequencyNumber();
