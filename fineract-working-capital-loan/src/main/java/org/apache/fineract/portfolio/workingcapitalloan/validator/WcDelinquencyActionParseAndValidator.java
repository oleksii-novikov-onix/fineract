...
@Component
public class WcDelinquencyActionParseAndValidator extends ParseAndValidator {
    ...
    public WcLoanDelinquencyAction validateAndParse(final JsonCommand command, final WorkingCapitalLoan wcLoan,
            final List<WcLoanDelinquencyAction> existing) {
        final WcLoanDelinquencyAction parsedAction = parseCommand(command);
        validateLoanIsActive(wcLoan);

        if (DelinquencyAction.PAUSE.equals(parsedAction.getAction())) {
            validatePause(parsedAction, wcLoan, existing);
        } else if (DelinquencyAction.RESCHEDULE.equals(parsedAction.getAction())) {
            validateReschedule(parsedAction, wcLoan);
        }

        return parsedAction;
    }

    private void validatePause(final WcLoanDelinquencyAction action, final WorkingCapitalLoan wcLoan,
            final List<WcLoanDelinquencyAction> existing) {
        validateBothDatesProvided(action);
        validateStartBeforeEnd(action);
        validateNotBeforeDisbursement(action, wcLoan);
        validateNotInEvaluatedPeriod(action, wcLoan);
        validateNoOverlap(action, existing);
    }

    private void validateReschedule(final WcLoanDelinquencyAction action, final WorkingCapitalLoan wcLoan) {
        validateLoanIsDisbursed(wcLoan);
        validateScheduleExists(wcLoan);
        validateMinimumPaymentProvided(action);
        validateFrequencyProvided(action);
    }

    private WcLoanDelinquencyAction parseCommand(final JsonCommand command) {
        final JsonElement json = command.parsedJson();
        final WcLoanDelinquencyAction action = new WcLoanDelinquencyAction();
    ...
    private void validateLoanIsActive(final WorkingCapitalLoan wcLoan) {
        if (!wcLoan.getLoanStatus().isActive()) {
    ...
    private void validateLoanIsDisbursed(final WorkingCapitalLoan wcLoan) {
        final boolean isDisbursed = wcLoan.getDisbursementDetails().stream()
    ...
    private void validateScheduleExists(final WorkingCapitalLoan wcLoan) {
        final List<WcLoanDelinquencyRangeSchedule> periods = rangeScheduleRepository.findByLoanIdOrderByPeriodNumberAsc(wcLoan.getId());
        if (periods.isEmpty()) {
    ...
    private void validateMinimumPaymentProvided(final WcLoanDelinquencyAction action) {
    ...
    private void validateFrequencyProvided(final WcLoanDelinquencyAction action) {
    ...
    private void validateBothDatesProvided(final WcLoanDelinquencyAction action) {
    ...
    private void validateStartBeforeEnd(final WcLoanDelinquencyAction action) {
    ...
    private void validateNotBeforeDisbursement(final WcLoanDelinquencyAction action, final WorkingCapitalLoan wcLoan) {
        if (action.getStartDate() == null) {
            return;
        }
        final LocalDate firstDisbursementDate = wcLoan.getDisbursementDetails().stream()
    ...
    private void validateNotInEvaluatedPeriod(final WcLoanDelinquencyAction action, final WorkingCapitalLoan wcLoan) {
        if (action.getStartDate() == null) {
            return;
        }
        final List<WcLoanDelinquencyRangeSchedule> periods = rangeScheduleRepository.findByLoanIdOrderByPeriodNumberAsc(wcLoan.getId());
    ...
    private void validateNoOverlap(final WcLoanDelinquencyAction parsed, final List<WcLoanDelinquencyAction> existing) {
    ...
    private boolean isOverlapping(final WcLoanDelinquencyAction parsed, final WcLoanDelinquencyAction existing) {
